...
func (ds *FetchDataSource) getModule(ctx context.Context, modulePath, vers string) (_ *fetch.LazyModule, err error) {
	...
	if m != nil && ds.opts.ProxyClientForLatest != nil && modulePath != stdlib.ModulePath {
...
type importedByGetter interface {
	ImportedBy(packagePath string) []string
}

func (ds *FetchDataSource) importedBy(packagePath string) []string {
	seen := make(map[string]bool)
	for _, getter := range ds.opts.Getters {
		if getter, ok := getter.(importedByGetter); ok {
			for _, importer := range getter.ImportedBy(packagePath) {
				seen[importer] = true
			}
		}
	}
...
func (ds *FetchDataSource) GetImportedBy(ctx context.Context, pkgPath, modulePath string, start string, limit int) ([]string, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	importers := ds.importedBy(pkgPath)
	if start != "" {
		first := sort.Search(len(importers), func(i int) bool { return importers[i] > start })
		importers = importers[first:]
	}
	if limit > 0 && len(importers) > limit {
