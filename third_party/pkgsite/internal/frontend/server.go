...
func NewServer(scfg ServerConfig) (_ *Server, err error) {
	...
	s := &Server{
		...
		disableExternalLinks:  scfg.DisableExternalLinks,
...
func (s *Server) Install(handle func(string, http.Handler), cacher Cacher, authValues []string) {
	...
	handle("GET /v1beta/package/", s.apiHandler(api.ServePackage))
	handle("GET /v1beta/symbols/", s.apiHandler(api.ServePackageSymbols))
	handle("GET /v1beta/imported-by/", s.apiHandler(api.ServePackageImportedBy))
	...
	handle("GET /api", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		http.Redirect(w, r, "/v1beta/api", http.StatusMovedPermanently)
	}))
	handle("GET /v1beta", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		http.Redirect(w, r, "/v1beta/api", http.StatusMovedPermanently)
	}))
	...
	handle("/v1/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		path := strings.Replace(r.URL.Path, "/v1/", "/v1beta/", 1)
