...
func BuildServer(ctx context.Context, serverCfg ServerConfig) (*frontend.Server, error) {
	...
	cfg := getterConfig{
		...
		proxyGetter:         serverCfg.ProxyGetter,
		goRepoPath:          serverCfg.GoRepoPath,
		disableRemoteStdlib: serverCfg.DisableRemoteStdlib,
	}
	...
	// Keep proxy-backed modules on the homepage without preloading them as
	// local modules at fetch.LocalVersion.
	seenModules := make(map[string]bool)
	var localModules []frontend.LocalModule
	for _, modules := range cfg.dirs {
		for _, m := range modules {
			if seenModules[m.ModulePath] {
				continue
			}
	...
	homepageModules := append([]frontend.LocalModule(nil), localModules...)
	for _, m := range serverCfg.AdditionalModules {
		if seenModules[m.ModulePath] {
			continue
		}
		seenModules[m.ModulePath] = true
...
type getterConfig struct {
	...
	proxyGetter         fetch.ModuleGetter                // custom proxy getter, or nil
	useLocalStdlib      bool                              // use go/packages for the local stdlib
	goRepoPath          string                            // repo path for local stdlib
	disableRemoteStdlib bool                              // never download a stdlib archive
}
...
func buildGetters(ctx context.Context, cfg getterConfig) ([]fetch.ModuleGetter, error) {
	...
	if cfg.proxyGetter != nil {
		getters = append(getters, cfg.proxyGetter)
	...
	if !usingLocalStdlib && !cfg.disableRemoteStdlib {
